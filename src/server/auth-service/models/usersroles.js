// OLHAR O RETORNO DAS FUNÇOES e conferir se realmente é o que se espera para a inserção de dados dos usuários

const connect = require('../config/database');

function getAdm() {

  var myHeaders = new Headers();
  myHeaders.append("Accept", "application/json");
  myHeaders.append("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IkM3TklpVGtiMUduUENEeDVHN0VOWiJ9..Ik50msYNhSx8QJegMzv6Tarcol05fdSMgPHgUCneEZ8a6XtU1o9xSu-6D53S5qhzIBk5JXh8FuDp8Zj7VU4HFNlUJ9D3jl1nErjw2ltE_gM7-QRdC9s3_1og-W-qzHyVDC9M9q_cOGZ6EXFTDoNXVRoL8gFA7ouwxgTdZDN3mNq79PAWGS_bYi9sl0LNLU6eCvCR0wz8vcL8Hqydfh5FxYVVek6Zv0CwAgzWhKP0bPssTpjMzJUgVdcsfHnCQHsLEvgJN_S1mfMgDhPUCkhL9xh_azxGVzBJvinuMCjvvCxjWPuxVQ7WsE_l3PZM51EhQ4utKeJOCzAbQ_FDNJGgoA");

  var requestOptions = {
    method: 'GET',
    headers: myHeaders,
    redirect: 'follow'
  };

  fetch("https://dev-d2d6navz25q1fi11.us.auth0.com/api/v2/roles/rol_v29cDvDN5h4xp7fK/users", requestOptions)
    .then(response => response.text())
    .then(result => console.log(result))
    .catch(error => console.log('error', error));
}

function getInspetor () {
  var myHeaders = new Headers();
  myHeaders.append("Accept", "application/json");
  myHeaders.append("Authorization", "Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IkM3TklpVGtiMUduUENEeDVHN0VOWiJ9..Ik50msYNhSx8QJegMzv6Tarcol05fdSMgPHgUCneEZ8a6XtU1o9xSu-6D53S5qhzIBk5JXh8FuDp8Zj7VU4HFNlUJ9D3jl1nErjw2ltE_gM7-QRdC9s3_1og-W-qzHyVDC9M9q_cOGZ6EXFTDoNXVRoL8gFA7ouwxgTdZDN3mNq79PAWGS_bYi9sl0LNLU6eCvCR0wz8vcL8Hqydfh5FxYVVek6Zv0CwAgzWhKP0bPssTpjMzJUgVdcsfHnCQHsLEvgJN_S1mfMgDhPUCkhL9xh_azxGVzBJvinuMCjvvCxjWPuxVQ7WsE_l3PZM51EhQ4utKeJOCzAbQ_FDNJGgoA");

  var requestOptions = {
    method: 'GET',
    headers: myHeaders,
    redirect: 'follow'
  };

  fetch("https://dev-d2d6navz25q1fi11.us.auth0.com/api/v2/roles/rol_7ChsmxraLANWECuI/users", requestOptions)
    .then(response => response.text())
    .then(result => console.log(result))
    .catch(error => console.log('error', error));
}




async function populateUsersRoles() {
  const db = await connect();
  const usersRolesCollection = db.collection('users');

  // Obter dados de ambas as funções
  const admins = await getAdm();
  const inspetores = await getInspetor();

  // Verifique se os dados são arrays válidos
  if (!Array.isArray(admins)) {
    console.error("Erro: admins não é um array");
    return;
  }
  if (!Array.isArray(inspetores)) {
    console.error("Erro: inspetores não é um array");
    return;
  }

  // Combina ambas as listas
  const users = [...admins, ...inspetores];

  for (const user of users) {
    // Verifica se o usuário já existe na coleção
    const existingUser = await usersRolesCollection.findOne({ userId: user.userId });
    if (!existingUser) {
      // Insere o novo usuário com sua role
      await usersRolesCollection.insertOne(user);
      console.log(`Novo usuário inserido: ${user.name} (${user.role})`);
    } else {
      console.log(`Usuário já existente: ${user.name} (${user.role})`);
    }
  }
}


module.exports = { populateUsersRoles };
